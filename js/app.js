...
function createFileCard(file) {
    const isCollected = StorageManager.isCollected(file.id);
    const user = StorageManager.getUser();
...
function openFile(fileId) {
    const file = StorageManager.getFiles().find(f => f.id === fileId);
...
// 切换收藏
function toggleCollected(fileId) {
    if (StorageManager.isCollected(fileId)) {
        StorageManager.removeCollected(fileId);
    } else {
        StorageManager.addCollected(fileId);
    }
    loadAllFiles();
}
...
function loadCollections() {
    const files = StorageManager.getFiles();
    const collected = StorageManager.getCollected();
...
function loadCategoriesView() {
    ...
    topLevelCategories.forEach(category => {
        const fileCount = StorageManager.getFiles()
...
function showCategoryWithChildren(categoryId, pushHistory = true) {
    const categories = StorageManager.getCategories();
...
// 显示分类下的文件（叶子分类）
function showCategoryFiles(categoryId, categories = null, fullPath = '') {
    const user = StorageManager.getUser();
    const allCategories = categories || StorageManager.getCategories();
    const category = allCategories.find(c => c.id === categoryId);
    const files = StorageManager.getFiles().filter(f => f.categoryId === categoryId);
...
function handleAddFile(event) {
    ...
    const newFile = {
        ...
        name: fileName,
        category: getCategoryPath(category, categories),
    ...
    populateCategorySelect(); // 保持下拉与分类结构同步
...
function populateCategorySelect(selectId = 'fileCategory') {
...
// 显示修改分类模态框（管理员）
function showChangeCategoryModal(fileId) {
    const user = StorageManager.getUser();
    ...
    populateCategorySelect('changeFileCategorySelect');
...
// 保存修改后的文件分类（管理员）
function handleChangeFileCategory(event) {
    event.preventDefault();
    ...
    file.categoryId = newCategory.id;
    file.category = getCategoryPath(newCategory, categories);
    StorageManager.setFiles(files);
    ...
    // 刷新当前视图
    if (currentTab === 'all-files') loadAllFiles();
    else if (currentTab === 'my-collections') loadCollections();
    else if (currentCategoryIdForFiles !== null) {
        showCategoryFiles(currentCategoryIdForFiles);
    }
}
...
// 创建分类树节点
function createCategoryTreeNode(category, allCategories) {
    const fileCount = StorageManager.getFiles()
        .filter(f => f.categoryId === category.id).length;
    ...
    if (children.length > 0) {
        ...
        children.forEach(child => {
            const childElement = createCategoryTreeNode(child, allCategories);
...
function deleteCategory(categoryId) {
    ...
    // 检查该分类下是否有文件
    const fileCount = StorageManager.getFiles()
        .filter(f => f.categoryId === categoryId).length;
    
    // 检查该分类下是否有子分类
    const childrenCount = categories.filter(c => c.parentId === categoryId).length;
    
    if (fileCount > 0) {
        alert(`⚠️ 该分类中还有 ${fileCount} 个文件，请先删除分类中的文件再删除分类。`);
        return;
    }
    
    if (childrenCount > 0) {
        alert(`⚠️ 该分类下还有 ${childrenCount} 个子分类，请先删除子分类再删除该分类。`);
        return;
    }
...
function searchCategoryFiles() {
    ...
    // 获取当前叶子分类下的所有文件
    const files = StorageManager.getFiles().filter(f => f.categoryId === currentCategoryIdForFiles);
    const categoryFilesList = document.getElementById('categoryFilesList');
